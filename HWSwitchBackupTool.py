...
from paramiko.ssh_exception import AuthenticationException, SSHException, ChannelException
...
def executeCommand(ip, port, username, password, command):
    ...
    try:
        ...
        print(f"连接到 {ip} 失败！SSH连接异常。")
    except ChannelException as ce:
        if ce.code == 60:
            print(f"连接到 {ip} 超时！跳过该连接。")
        else:
            print(f"执行命令失败！错误码：{ce.code}")
    except Exception as e:
        print(f"连接到 {ip} 失败！{str(e)}")
